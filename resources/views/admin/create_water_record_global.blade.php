...
@section('title', 'Log Reading - Water Systems')
@section('header_title', 'Log Daily Water Consumption')
        ...
        <div class="card">
            ...
            <form action="{{ route('admin.water-records.submit') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="nic" class="form-control" required autofocus>
                        <option value="">-- Choose registered user --</option>
                        @foreach($users as $user)
                            <option value="{{ $user->nic }}" {{ old('nic') == $user->nic ? 'selected' : '' }}>
                                {{ $user->name }} (NIC: {{ $user->nic }})
                            </option>
                        @endforeach
                    </select>
                </div>
